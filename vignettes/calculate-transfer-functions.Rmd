...
vignette will still build, though, but without actually calculating any time
...
target resolution of our proxy data:
```{r}
dtf$dml1 <- tmp # copy first to preserve attributes
dtf$dml1$freq <- ff$dml1
dtf$dml1$spec <- approx(tmp$freq, tmp$spec, dtf$dml1$freq)$y
```

Run the time uncertainty model with Gaussian white noise as surrogate data:
```{r}
set.seed(4122018)
if (has.simproxyage) {
  suppressMessages(
