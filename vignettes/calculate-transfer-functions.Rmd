...
the parameter `sigma` in the call of the function `CalculateDiffusionTF` below,
and `?CalculateDiffusionTF`). Along with **proxysnr**, the diffusion length
estimates for the DML and WAIS cores are provided in `diffusion.length`; for
details see `?diffusion.length`. Calculation of these estimates is documented in
the package source under `./data-raw/calculate-diffusion-length.R`, where `.` is
the root of the package source directory. The relevant site parameters needed
for the diffusion length calculations are provided in `clim.site.par`; for
details see `?clim.site.par`. For further information see the publication Münch
and Laepple (2018).
